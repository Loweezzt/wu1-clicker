...
let achievements = [
    {
        description: 'Now the fun starts',
        requiredUpgrades: 1,
        acquired: false,
    },
    {
        description: 'Gettin started',
        requiredUpgrades: 10,
        acquired: false,
    },
    {
...
function createCard(upgrade) {
    ...
    if (upgrade.amount) {
        header.textContent = `${upgrade.name}, +${upgrade.amount}/s`;
    } else {
        header.textContent = `${upgrade.name}, +${upgrade.clicks}/click.`;
    }
    ...
    card.addEventListener('click', (e) => {
        if (money >= upgrade.cost) {
            ...
            message('insufficient funds', 'warning');
        }
    });
